...
0),r=(m?m+",":"")+(h?h+",":"")+(p?p+",":"")+(q?q:"");r?(e.mmInitCallback=function(a){a(k,r,g)},F("MM.PRUH",a.baseContentUrl+"utils/pruh.js")):g()}})(a);return this}({
    ...
    beforeInit: function (getParam, setParam, loader) { 
        // cross domain solution - postMessage/iframe version
        function getRootDomain(hostname) {
            return hostname.match(/^[\d.]+$|/)[0] || ((hostname.match(/[^.]+\.(\w{2,3}\.\w{2}|\w{2,})$/) || [hostname])[0]);
        }

        var FRAME_QUERY = '?mmcrossdomainsolution=yzOaGH52SiHbe4M';

        // iframe domains, array of objects {hostname:'domain.noprotocol.com', pathname:'/static-page'}
        var domainsList = [{ hostname: 'www.rocketaccount.com', pathname: '/maxymiser-track/index.html' }, { hostname: 'www.rocketmortgage.com', pathname: '/nsassets/rm/blank.html' }, { hostname: 'www.quickenloans.com', pathname: '/nsassets/ql/blank.html' }];
        var canUpdateDomainsList = true;

        window.mmCrossDomain = {};

        window.mmCrossDomain._transports = [];

        if (!window.mmCrossDomain._iframes) {
            window.mmCrossDomain._iframes = {};
        }
        var insertIframe = function(hostname, pathname) {
            if (window.mmCrossDomain._iframes[hostname]) {
                return;
            }
            var frameSrc = '//' + hostname + pathname + FRAME_QUERY;

            var frameEl = document.createElement('iframe');

            frameEl.setAttribute('src', frameSrc);
            frameEl.setAttribute('class', 'mm-cross-domain');
            frameEl.setAttribute('style', 'opacity:0; position:absolute; top:-100000px; left:-100000px; width:0; height:0;');

            window.mmCrossDomain._iframes[hostname] = frameEl;

            document.body.appendChild(frameEl);
        };

        var insertIframes = function(domains) {
            if (document.body.childNodes.length && typeof document.body.appendChild === 'function' && typeof domains.forEach === 'function') {
                domains.forEach(function(domain) {
                    if (getRootDomain(window.location.hostname) === getRootDomain(domain.hostname)) {
                        return;
                    }

                    insertIframe(domain.hostname, domain.pathname);
                });
            ...
            setTimeout(function() {
                insertIframes(domains);
            }, 100);
        };

        if (window.location.search.indexOf(FRAME_QUERY) !== -1) {
            if (window.stop) {
                window.stop();
            } else {
            ...
            window.top.postMessage('mm-ready-cross-domain', '*');

            window.addEventListener('message', function(event) {
                if (typeof event.data === 'string') {
                    if (event.data.indexOf('mm-write-param') > -1) {
                        var paramList = event.data.split('&!&');
                        setParam(paramList[1], paramList[2], 0);
                    } else if (event.data.indexOf('mm-write-data') > -1) {
                        var dataParamList = event.data.split('&!&');
                        localStorage.setItem(dataParamList[1], dataParamList[2]);
                    }
                }
            }, false);
            ...
            window.addEventListener('load', function load() {
                canUpdateDomainsList = false;

                if (!domainsList.length) {
                    return;
                }

                var validDomainsList = domainsList
                    .filter(function(domain, index, array) {
                        return array.map(function(domain) {
                            return getRootDomain(domain.hostname);
                        }).indexOf(getRootDomain(domain.hostname)) === index;
                    });

                insertIframes(validDomainsList);
            }, false);

            window.addEventListener('message', function(event) {
                if (event.data !== 'mm-ready-cross-domain') {
                    return;
                }

                window.mmCrossDomain._transports.push(event.source);
            }, false);
        }

        window.mmCrossDomain.add = function(domain) {
            if (Object.prototype.toString.call(domain) !== '[object Object]' ||
                typeof domain.hostname !== 'string' ||
                typeof domain.pathname !== 'string') {
                return;
            }

            if (canUpdateDomainsList) {
                domainsList.push(domain);
            }
        };

        window.mmCrossDomain.transferData = function(key, value) {
            var transports = window.mmCrossDomain._transports;
            var iframes = window.mmCrossDomain._iframes;
            var domains = Object.keys(iframes);

            if (!transports.length || transports.length !== domains.length) {
                setTimeout(function() {
                    window.mmCrossDomain.transferData(key, value);
                }, 100);
                return;
            }

            if (domains && typeof domains.forEach === 'function') {
                domains.forEach(function(domain) {
                    iframes[domain].contentWindow.postMessage('mm-write-data&!&' + key + '&!&' + value, '*');
                });
            }
        };
    },
    ...
    afterResponse: function (getParam, setParam, genInfo) { 
        ...
        var paramsToTransfer = ['pd', 'srv', 'mmid'];
        var transports = window.mmCrossDomain._transports;
        var iframes = window.mmCrossDomain._iframes;

        var transferParam = function(param) {
            for (var i = 0; i < transports.length; i += 1) {
                transports[i].postMessage('mm-write-param&!&' + param + '&!&' + getParam(param, 0), '*');
            }
        };

        var transferParams = function() {
            if (!transports.length || transports.length !== Object.keys(iframes).length) {
                setTimeout(transferParams, 100);
                return;
            }

            for (var i = 0; i < paramsToTransfer.length; i += 1) {
                transferParam(paramsToTransfer[i]);
            }
        };
